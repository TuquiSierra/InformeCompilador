...
\subparagraph{Representación de tipos:}
	Los tipos se representan en memoria de la siguiente forma:
...
La relación de herencia se determina a través de un atributo llamado \textit{order}, creado desde la generación de código intermedio como atributo especial. Si A hereda de B, entonces seguramente $order(A)<order(B)$. 

De esta forma garantizamos que en la ejecución de una instrucción \textit{case}, podamos decidirnos por la rama cuyo $order(type)$ se el menor tal que $order(type)\geq order(type(expression))$. 
